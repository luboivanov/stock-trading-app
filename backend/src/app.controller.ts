...
export class AppController {
  ...
  getBestTrade(
    ...
    if (!firstTimestamp) {
      throw new BadRequestException('No data available: CSV is missing start timestamp.');
    }

    if (!lastTimestamp) {
      throw new BadRequestException('No data available: CSV is missing end timestamp.');
    }

    if (start < firstTimestamp) {
      throw new BadRequestException(`Start time is before available data range. Earliest available timestamp is ${firstTimestamp.toISOString()}.`);
    }

    if (end > lastTimestamp) {
      throw new BadRequestException(`End time is after available data range. Latest available timestamp is ${lastTimestamp.toISOString()}.`);
    }
    ...
    // Optionally compute profit/stocks here, in frontend instead
    if (fundsStr) {
